...
def configure_logger(ds, logdir):
...
def save_results(y_preds, ds, resdir, n_folds=10):
...
def save_results_change_censoring(y_preds, resdir, tag, features):
    print(y_preds.shape)
    print(len(features))
    df = pd.DataFrame(y_preds.transpose()) #, columns=features)
    df.to_csv(f'{resdir}/final_results_{tag}_support.csv', index=False)